...
The prototype now includes a searchable catalog of 320 AWS services across 21 categories. Its network library supports freely named and color-coded workflow sections, VPC and Availability Zone boundaries, public and private subnets, NAT and Internet gateways, route tables, network ACLs, security groups, VPC endpoints, and external workflow actors. The full editor opens as a blank workspace: services, logical sections, and network boundaries can be dragged from the library to an exact position, freely repositioned, resized on-canvas or from Properties, and explicitly deleted. Services can be placed inside any logical or network container. Manual arrows can connect services, sections, VPCs, or subnets and can be labeled as a data flow, NAT route, VPN tunnel, VPC peering link, Transit Gateway route, Direct Connect link, VPC Endpoint, Internet Gateway path, Internet route, or custom connection.
